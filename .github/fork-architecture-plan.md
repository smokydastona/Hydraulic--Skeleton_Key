...
- Pack module discovery now precedes resource-pack parsing. Hydraulic registers all `PackModule` services before deciding whether parsed `ResourcePack` objects are needed, and skips the `MinecraftResourcePackReader`/`NioDirectoryFileTreeReader` pass entirely when no loaded module has preprocessors. Preprocessing now receives an empty collection when an individual mod root cannot be read, preserving graceful degradation. This composes module discovery with resource loading instead of parsing every mod root before the consumer set is known. The active Fabric modules currently do use preprocessors, so the present dev pack still requires the parse; the optimization removes that cost for module sets that do not.
